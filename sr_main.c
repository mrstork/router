...
int main (int argc, char **argv) {
    ...
    unsigned int icmp_qt = DEFAULT_ICMP_QUERY_TIMEOUT;
    unsigned int tcp_eit = DEFAULT_TCP_ESTABLISHED_IDLE_TIMEOUT;
    unsigned int tcp_tit = DEFAULT_TCP_TRANSITORY_IDLE_TIMEOUT;
    ...
    while ((c = getopt(argc, argv, "nI:E:R:hs:v:p:u:t:r:l:T:")) != EOF) {
        switch (c) {
            ...
            case 'n':
                ...
                break;
            case 'I':
                icmp_qt = atoi((char *) optarg);
                break;
            case 'E':
                tcp_eit = atoi((char *) optarg);
                break;
            case 'R':
                tcp_tit = atoi((char *) optarg);
                break;
    ...
    if (template_name == NULL) {
        ...
        strncpy(sr.template_name, template_name, 30);
    }

    /* -- set up nat timings and attach to sr -- */
    if (nat_enabled) {
        struct sr_nat nat;
        sr_nat_init(&nat);
        sr_nat_init_timings(&nat, icmp_qt, tcp_eit, tcp_tit);
        sr.nat = &nat;
    } else {
        sr.nat = NULL;
    }
    ...
    if (template_name != NULL && strcmp(rtable, "rtable.vrhost") == 0) { /* we've recv'd the rtable now, so read it in */
